...
async def user_auth(refresh_token, user_id):
    """Update user track and token info.

    ### Path Parameter
    'refresh_token': string of alphanumeric values, and nothing more

    'user_id': The API requires the user_id in order to obtain pertinant data
    for analysis.
...
async def compatabilities(user_id_1, user_id_2):
    ...
    'compatability score':
    """
    return {"The Compatability score for two distinct users"}


# Generates user playlist
@router.post('/new_playlist/{playlistname}')
async def new_playlist(playlistname):
